...
**Check out the Demo here**: https://signature-based-minting.web3sdks-template.com/

## Tools:

- [**web3sdks React SDK**](https://docs.web3sdks.com/react): to enable users to connect and disconnect their wallets to our website, using [useMetamask](https://docs.web3sdks.com/react/react.usemetamask) & [useDisconnect](https://docs.web3sdks.com/react/react.usedisconnect), and prompt them to approve transactions with MetaMask. We'll also use the helpful hooks such as [useNFTs](https://docs.web3sdks.com/react/react.usenfts) to easily fetch data from the smart contract.
- [**web3sdks NFT Collection**](https://docs.web3sdks.com/contracts/nft-collection): to create an ERC721 NFT Collection that our community can mint NFTs into.
- [**web3sdks TypeScript SDK**](https://docs.web3sdks.com/typescript): to connect to our NFT Collection Smart contract via React hooks such as [useNFTCollection](https://docs.web3sdks.com/react/react.usenftcollection), mint new NFTs with [signature based minting](https://docs.web3sdks.com/typescript/sdk.nftcollection.signature).
...
```bash
npx web3sdks create --template signature-based-minting
```

- Create your own NFT Collection via the web3sdks dashboard (follow the steps in **Setting Up The NFT Collection** of this doc if you need more help)
...
To create an NFT collection, we can use the web3sdks dashboard and create a fully customizable NFT collection in just a few clicks.

Head to https://web3sdks.com/dashboard and create a new contract on the Polygon Mumbai network. Click **Create NFTs and Tokens** > **NFT Collection**.

Give your collection a name and click **Deploy now**!

Nice! Now we have an NFT collection. Let's set up a project and see how we can start minting some NFTs using the web3sdks SDK.

## Setting Up The Project

To get started, we have a ready-made template that includes all the code you need to work with Web3sdks, TypeScript and Next JS available here https://github.com/web3sdks-template/next-typescript-starter

Our application is wrapped in a Web3sdks Provider so that we can access Web3sdks anywhere in our application:
...
Over at the home page at `index.tsx`, we're using the [web3sdks React SDK MetaMask Connector](https://docs.web3sdks.com/react/category/wallet-connection) so our user can connect their wallet to our website.
...
For our community collection, each NFT is minted by a different user. We're using another web3sdks hook called `useNFTCollection` to connect to our smart contract and load all of the NFT's in our collection.

```tsx
// Fetch the NFT collection from web3sdks via it's contract address.
...
**Initialize the Web3sdks SDK on the server-side**

```tsx
// Initialize the Web3sdks SDK on the serverside
